	...
	program
		...
		.action(async (options: any) => {
			...
			if (!message) {
				...
				if (header !== null) {
					message = await nameSuggestion(header === "" ? undefined : header);
			...
			git
				...
				.then((result) => {
					...
					signale.info(`Changes:    ${chalk.yellow(result.summary.changes)}`);
					signale.info(`Insertions: ${chalk.green(result.summary.insertions)}`);
					signale.info(`Deletions:  ${chalk.red(result.summary.deletions)}`);
				})
				.catch((e) => {
